---
id: S-02
title: "CPM v1.1 features - inventory, retention, re-associate (F13-F15)"
type: spec
status: committed
created: 2026-07-11
updated: 2026-07-11
...
# S-02: CPM v1.1 features - inventory, retention, re-associate (F13-F15)
...
- **F15 `cpm associate --from A --to B` (re-associate/export)** - keep A's Claude
...
| AC | Summary |
|----|---------|
| AC-28 | `cpm list` enumerates every project dir, resolves each to stored `cwd`, flags no-cwd dirs UNRESOLVED rather than omitting them |
...
| AC-34 | `cpm archive --archive-dir D` copies every project's transcripts to `D/projects/<enc>/` byte-for-byte; a re-run copies nothing unchanged (content-hash incremental) |
...
| AC-40 | `cpm associate --from A --to B` locates A's sessions via stored `cwd` even when A's folder no longer exists on disk |
